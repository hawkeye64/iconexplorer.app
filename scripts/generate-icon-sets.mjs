...
// Minimum published package version that first exposed each icon set.
// Sets available since quasar-extras-svg-icons v1.0 intentionally omit a badge.
const statusByIconSet = new Map([
  ...
  ['quasar-extras-svg-icons:cool-icons-v4', 'v1.31'],
  ...
  ['quasar-extras-svg-icons:health-icons-v1', 'v1.37'],
  ['quasar-extras-svg-icons:health-icons-v2', 'v2.0'],
  ['quasar-extras-svg-icons:hero-icons', 'v1.8'],
  ...
  ['quasar-extras-svg-icons:material-line-icons', 'v1.6'],
  ['quasar-extras-svg-icons:material-line-icons-v1', 'v1.37'],
  ...
  ['quasar-extras-svg-icons:tabler-icons-v3', 'v1.37'],
  ['quasar-extras-svg-icons:teeny-icons', 'v1.2'],
  ['quasar-extras-svg-icons:typ-icons', 'v1.11'],
...
function packageIconSetPath(pkg) {
  return resolve(packagePath(pkg.packageName), pkg.sourceDirectory ?? '')
}

function packageImportBasePath(pkg) {
  return ['../../node_modules', pkg.packageName, pkg.sourceDirectory].filter(Boolean).join('/')
}

function readIconSetFolders(pkg) {
  const baseDir = packageIconSetPath(pkg)
  ...
  return readdirSync(baseDir, { withFileTypes: true })
    ...
    .filter(
      (name) =>
        ...
        existsSync(resolve(baseDir, name, pkg.moduleFile)),
    )
}
...
function generateIconSets() {
  return packages.map((pkg) => {
    const folders = readIconSetFolders(pkg)
...
function formatGlobEntries(iconSets, pkg, fileName) {
  const importBasePath = packageImportBasePath(pkg)

  return iconSets
    .find((group) => group.label === pkg.label)
    .children.map((iconSet) => `  '${importBasePath}/${iconSet.value}/${fileName}',`)
    .join('\n')
}

function writeIconModulesFile(iconSets) {
  const extrasPackage = packages.find((pkg) => pkg.packageName === '@quasar/extras')
  const quasarExtrasSvgIconsPackage = packages.find(
    (pkg) => pkg.packageName === 'quasar-extras-svg-icons',
  )

  const output = `// This file is generated by scripts/generate-icon-sets.mjs.
// Run \`pnpm generate:icon-sets\` after updating icon packages.

export type IconNamePayload = string[] | Record<string, unknown> | string
export type IconJsonModule = { default: IconNamePayload }
export type IconModule = Record<string, string>

export const extrasJson = import.meta.glob<IconJsonModule>([
${formatGlobEntries(iconSets, extrasPackage, 'icons.json')}
])

export const extrasModules = import.meta.glob<IconModule>([
${formatGlobEntries(iconSets, extrasPackage, extrasPackage.moduleFile)}
])

export const quasarExtrasSvgIconsJson = import.meta.glob<IconJsonModule>([
${formatGlobEntries(iconSets, quasarExtrasSvgIconsPackage, 'icons.json')}
])

export const quasarExtrasSvgIconsModules = import.meta.glob<IconModule>([
${formatGlobEntries(iconSets, quasarExtrasSvgIconsPackage, quasarExtrasSvgIconsPackage.moduleFile)}
])
`
